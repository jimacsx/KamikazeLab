<?php
  ...
  $cols = array('', 'nombreDesarrollo', 'nombreContacto', 'telefonoContacto', '', '', '', '');
  ...
  $r=dbRow('select count(iddesarrollo) as d from desarrollo');
  $total_records=$r['d']; 
  ...
  */
  ...
  if(isset($_REQUEST['sSearch']) && ''!=$_REQUEST['sSearch']){ 
    ...
    $search_sql='where '; 
    if(strlen($stext)) {
      $search_sql.="nombreDesarrollo like '%$stext%' or "; 
  ...
  if($search_sql){ 
    $r=dbRow("select count(iddesarrollo) as d from desarrollo $search_sql"); 
  ...
  $rs=dbAll("select iddesarrollo, logo, nombreDesarrollo, nombreContacto, telefonoContacto, iddesarrollador from desarrollo $search_sql
  ...
  foreach($rs as $r){ 
    ...
    echo '["',"<center><img src='".$r['logo']."'></center>",'",
           "',addslashes($r['nombreDesarrollo']),'",
           "',$r['nombreContacto'],'",
           "',$r['telefonoContacto'],'",
           "',mysql_num_rows(mysql_query("SELECT * FROM desarrollador NATURAL JOIN casa WHERE iddesarrollador = '{$r['iddesarrollador']}'")),'",
           "',"",'",
           "',mysql_num_rows(mysql_query("SELECT * FROM desarrollo WHERE iddesarrollador = '{$r['iddesarrollador']}'")),'",
